...
1. Read this file. If the change is contained to one source module, match that module's existing patterns before editing.
...
- (CI also runs `connectedAndroidTest` for instrumentation tests; trust the CI signal for those)

PR shape (matches this repo's `CONTRIBUTING.md`):

- **Branch**: descriptive name (e.g. `fix/session-timeout-handling`).
